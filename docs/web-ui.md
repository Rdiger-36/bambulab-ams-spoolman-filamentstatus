...
On a phone each unit is one card, with that header on top and its slots as sections inside it. Above each unit's table stands what the unit reports about itself: which unit it is, the relative humidity inside it, its temperature, and, while a drying cycle runs, its target temperature and the minutes left. The name is the one the printer gives in its module list, which the service asks for on every connection: **AMS**, **AMS Lite**, **AMS 2 Pro** or **AMS HT**. The status report itself cannot tell them apart, an original AMS on current firmware sends the same fields as a 2 Pro, dryer fields included. Until the answer arrives a single slot unit reads **AMS HT** and everything else plain **AMS**. Which readings appear depends on the model. An AMS 2 Pro and an AMS HT report all of it. The original AMS and the AMS Lite report only the five step humidity level, 1 being the driest, and have no temperature sensor. The AMS Lite has no humidity sensor either and always reports 5, so take that level for what it is. The external spool holder reports none of it and gets no header.
...
The same bar sits on every page and carries two things: where you can go, and your session. Left the four pages with the one you are on marked, right the dark and light mode switch and, once a [password](settings.md#the-web-ui-password) is set, **Log out**. **Logs** lists the server log and one entry per printer, by name, so the log you open is the one you picked rather than the one belonging to whichever printer was selected last. **API** opens every route of the service with a form to send it from, see the [API page](api.md). On a phone the four pages fold into one button.

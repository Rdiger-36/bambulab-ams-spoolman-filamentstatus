...
The same choice is available to a script through the `scope` query of `GET /api/diagnostics/download`: `server` and serial numbers, comma separated, for example `?scope=server,01P00A000000042`. Without it the bundle carries every log.

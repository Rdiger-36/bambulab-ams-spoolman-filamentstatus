...
Unreleased
   ...
   - New Features:
      ...
      - The rest of the Web UI follows the dashboard onto a phone: the printer list and the API keys are cards with their labels above the values instead of a table behind a sideways scroll, where the status and the buttons were out of reach; the facts of the Service card sit two to a line; a dialog fits inside the screen instead of hanging 12 px over its right edge; each spool in the assign dialog is two lines, its name and under it the weight and the badges, both starting at the same edge next to the radio button, instead of one line whose parts wrapped apart; a new API key gets the full width with its copy button under it; and the footer no longer runs past the right edge, which cut the last words of its line

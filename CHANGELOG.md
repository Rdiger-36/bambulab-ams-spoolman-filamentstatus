...
Version 1.3.0-dev
   - New Features:
      ...
      - Settings page in the Web UI: everything that used to be an environment variable can now be changed in the browser, plus the printer list
         ...
         - server.log is appended to instead of being replaced on every start, so a restart no longer takes the lines with it that explain why it happened. It is trimmed to its last 2000 lines once it passes 1 MB

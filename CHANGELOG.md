...
Version 1.3.0-dev.22
   ...
   - Fixes:
      - The expected end of a running print stands still. It was the printer's remaining minutes added to the clock at the moment the dashboard asked, and the dashboard asks with every report while the printer revises the minutes about once a minute, so the end crept forward a few seconds at a time and jumped back on every revision. Seen on an X1C on 2026-09-21 with a print that ran over midnight, where the long form of the time showed the seconds counting up. The end is counted from the moment the printer last revised the figure now, and it is written without seconds in every form, because whole minutes are all the printer knows

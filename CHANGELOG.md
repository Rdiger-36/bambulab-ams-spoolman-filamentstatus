...
Unreleased
   ...
   - Fixes:
      ...
      - A print is matched to the right slot when a slot in the middle of an AMS is empty. Bambu Studio leaves an empty slot out of the filament list when it synchronises a project with the printer, so every filament after a gap moves up one position, while this service counted all four slots of every unit and looked one or two positions too far to the right. Measured on a P2S emptied at A2 and B1: seven loaded slots, seven filaments, both gaps absent
         ...
         - Nothing was booked onto the wrong spool by this, because a slot that does not hold what was sliced refuses the position and the filament is then placed by its profile and colour. That fallback cannot tell two identical spools apart, though, and the position now can
   - Development:

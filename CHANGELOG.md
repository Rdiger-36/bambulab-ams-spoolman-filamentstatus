...
Version 1.3.0-dev.13
   ...
   - Features:
      - Every MQTT report a printer sends can be captured into a file of its own, "Capture raw MQTT messages" in the log detail dialog. It is what the printer really sent rather than what this service made of it, which is what a bug report about behaviour nobody can reproduce on demand needs
         ...
         - Written one line per report into logs/<serial>.mqtt.log, with its own size and history budget: at the log file's default of 1 MB a capture would evict the whole log within minutes. Measured on a P2S it runs at about 21 MB an hour, a full report every 1.4 seconds, so the default of 50 MB with 2 kept files is roughly six hours of history and the dialog says what an hour costs

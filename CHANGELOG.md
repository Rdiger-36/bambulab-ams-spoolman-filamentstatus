...
Version 1.3.0-dev.11
   - Features:
      ...
      - The card says more about a print while it is running: when it started, how long it has been going and when it is expected to end, next to the layer progress that was already there
         ...
         - How long it has been running counts up in the same clock as the countdown next to the booking label, and the time left is shown as a duration rather than as the raw minutes the printer reports: "21:30:00 left" instead of "1290 min". The start and the expected end follow the same rule as the summary: the times alone while the whole job falls on today, the full date once it runs over midnight

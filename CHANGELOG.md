...
Unreleased
   ...
   - Fixes:
      - Dark mode reaches the controls the browser draws itself. The option list of a select, the scrollbars, the radio buttons and the spin buttons were painted light over a dark page, which on macOS produced a white popup with grey text in it. The page now declares which scheme it is in, which is the only lever on those. The colour panel of the operating system stays what the system is set to, since it is a window of its own and no page can reach it

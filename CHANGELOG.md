...
Version 1.3.0-dev.4
   - Fixes:
      ...
      - Filament consumption is booked onto the right spool when two loaded spools look alike
         ...
         - The order is by AMS unit id, which is what the printer numbers them by: the four slot units first, then an AMS HT, then the external spool holder. Read off a printer, whose nine reported slots matched the nine filaments of a sliced file position for position

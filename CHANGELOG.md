...
Version 1.3.0-dev
   - New Features:
      ...
      - Settings page in the Web UI: everything that used to be an environment variable can now be changed in the browser, plus the printer list
         ...
         - The service can be restarted from the settings page, which is what legacy mode needs. The process ends so that Docker or the Home Assistant supervisor starts it again, and the page waits for it to come back and reloads itself. Without a restart policy on the container the service stays down, which the page says before it asks

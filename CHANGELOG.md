...
Unreleased
   - Changes:
      - The print card says PREPARE instead of RUNNING while the printer is heating, homing, levelling or calibrating. The printer reports RUNNING from the first second of a job, and "RUNNING" next to "Homing toolhead" read like a contradiction; the stage badge next to it still names what the printer is doing
   ...
   - Fixes:
      - The sliced file of a print is looked for again when the first fetch found nothing, 30 seconds later and up to three times in all. The file was fetched exactly once, when the print reached RUNNING, and a printer that did not answer just then lost the whole print: a P2S recovering from an extruder error on 2026-09-23 let the FTPS login time out for 20 seconds, and the job was never booked although the file was there a minute later. A file the printer is still copying to its USB stick is caught the same way. The log says "trying again in 30 seconds" after a failed attempt and "consumption tracking unavailable" after the last, and the print card says the same: "Sliced file not found yet, looking again (attempt 2 of 3)" while attempts are left, "No sliced file on the printer, nothing will be booked for this print" in red after the last, and "✖ no sliced file" as the label of the finished print. The API carries it as sliceFetch on /api/print/{printerId}
      - The log counts layers the way the dashboard does. "Slice info loaded: 1 filament(s), 0 layers" for a one layer plate was the last layer's index, not the count
      - The dashboard no longer shows the whole plate as "booked" for a print cancelled before its first layer. The booking wrote 0 g and the spool was left alone, but the row under "Needed" fell back to the full amount when the used amount was 0; seen on a P2S on 2026-09-23 after a cancel at layer 0. The card's label over such a print reads "nothing to book", in grey, where it said "consumption booked" for a booking that never happened
...
Version 1.3.0-dev.23
   - Fixes:
      ...
      - The expected end of a print no longer counts seconds. It is "now plus the whole minutes the printer reports", so its seconds were those of the clock at the moment of the request and moved on with every refresh of the dashboard, visible as soon as a print ran over midnight and the moment was written with its date. Seen on 2026-09-21 on an X1C that had been running since the day before. A moment is written without seconds now, on the print card and in the summary dialog, and the API cuts the estimate to the whole minute the printer's number has
...
Version 1.3.0-dev.20
   - Features:
      - The API has a page of its own in the Web UI, opened with the info button next to the API keys on the settings page: every route with its parameters, an example body and the shapes of its answers, and a "Send" button per route that sends the request from the browser and shows the status, the headers and the answer. A path parameter is prefilled with a real serial number, the event stream is followed live, a download opens as one, and the routes that end the process or remove something ask first. A bar that stays at the top carries a filter over the list, and a curl line under every request carries the key typed at the top so it can be copied into a script as it will be run there
         ...
         - The page is dependency free like the rest of the Web UI and works on an installation without internet access. The three pages next to it were the model, not Swagger UI
   - Fixes:
      - The service starts on Windows. starting.js imported backend.js through its absolute path, and Node's module loader reads "D:\app\backend.js" as a URL with the scheme "d:" and refuses it, so "node entrypoint.js" ended in ERR_UNSUPPORTED_ESM_URL_SCHEME before anything ran. Both dynamic imports go through a file URL now, which every platform accepts
...
Version 1.3.0-dev.19
   - Fixes:
      ...
      - The slot update interval counts from every pass that ran. A pass that processed a change in G-code mode left the clock alone, so the very next report fetched Spoolman again for a comparison the interval is there to space out
      - The Studio echo a P2S sends twice within a second is logged once
      - Five settings added after the environment was deprecated can be seeded from it like every other: AUTO_ASSIGN_THIRD_PARTY, PRINT_RESET_MINUTES, ARCHIVE_EMPTY_SPOOLS, EMPTY_SPOOL_THRESHOLD and OFFLINE_MAX_INTERVAL. Set in a compose file they did nothing, without a word in the log, because the seed list in src/config.js had not been extended; a test now holds it against the schema
      ...
      - A cancelled print books the layers that were finished, not two more. The printer's layer_num is the layer being printed, counted from 1, and the booking read it as the number of the last finished layer counted from 0, so a cancel before the first layer booked one layer and a cancel at layer N booked N+1. Measured on a P2S through the raw trace: layer_num went to 1 the second printing began, after two and a half minutes of calibration at 0, and a ten layer plate ended at 11
         - Within a job the layer counter only goes up, and a job starts at 0. The P2S reports a stale layer in one report type next to the current one in the other, 4, 3, 4 within a second, and the report that starts a job still carries the previous job's last layer, 11 five seconds before the new job's 0; a cancel right after a stale value would have booked a layer too few, and the dashboard showed "Layer 11 / 10" at the start of a fresh print
      - "After print" no longer subtracts the print a second time once it is booked. The column showed on spool minus needed for as long as the result card stood, and after the booking the spool's weight already carried the print, so every spool read lighter than it is. Once booked, "Needed" says what was booked and "After print" is the spool's weight now
      ...
      - Web UI: "Go to Spoolman" in the no-match dialog opens Spoolman's create page when Spoolman lives under a path. The link was the footer's address plus "spool/create" without a slash between them, which is fine for "http://host:7912/" and "http://host/spoolmanspool/create" for "http://host/spoolman"; it is built like the other links now
      - Web UI: a filament or spool name, a slot's brand text, a log line and a printer name in the export dialog are shown as text. Each of those went into the page as markup, so a "<" in a name cut the line short, and a name with a tag in it ran the tag; the rest of the page already escaped them
      - An original AMS is not offered a dryer. Whether a unit shows a drying section followed the presence of the dryer fields in the report, and an original AMS on current firmware sends them: an X1E's AMS08 sent "dry_time: 0" in every report of a trace read on 2026-09-08. The model from the printer's module list decides now, an AMS 2 Pro and an AMS HT have a dryer, an AMS and an AMS Lite do not, and the report decides only until that answer is in
      - Web UI: the merge dialog rounds the grams left on the spool to two decimals ("640.00g", not "640.0000000001 g"), the spool dialog shows Remaining and Used to the hundredth like the table does, the no-match dialog is spelled right ("matching"), the footer says the current year, and the two theme icons in the menu bar are drawn in the page rather than fetched from icons8.com, which an installation without internet access showed as two broken images
   - Development:
      ...
      - The weight a slot carries to the dashboard has one meaning: "amsWeight" is what the AMS reads off the RFID tag, Bambu Lab spools only. It was "correctedWeight", and held that figure for a Bambu Lab spool but Spoolman's remaining weight for a 3rd party or an archived spool, and was overwritten with Spoolman's figure whenever the spool was edited, so the dashboard had to know where the number came from. It reads Spoolman's weight off the linked spool itself now, by tag, by assignment or as the archived spool still in the slot, and the AMS figure otherwise
...
Version 1.3.0-dev.18
   ...
   - Fixes:
      - The print card no longer says "consumption booked" over a print that booked nothing. The label followed the flag that says the booking ran, not what it did, so a print from a slot nobody had assigned ended with every filament skipped and a green tick above it. The label now counts the report's rows: "nothing booked" in red when no filament could be booked, "1 of 3 booked" in amber when some could not, and "consumption booked" only when every used filament was. The report behind it is unchanged and says why
      - A print on a P1 or an A1 is booked onto the slots Bambu Studio sent it to, not onto an estimate (issue #146). Those printers never report where a print runs from, and the estimate from the slicer's list order is only right for a project that was synchronised with the printer before slicing; a reused project is remapped by colour when the job is sent, and the list order then names the wrong slots. The printer echoes the command carrying that remapping on its report topic, and the service now reads it: the log says "Bambu Studio sent "Zylinder" to the slots ["A1","A2","A4"]" and the booking follows it
         - Read off three P1S prints and an X1E print, cloud and LAN alike. On the X1E the echo and the printer's own report agreed; on a P1S with a reused project the echo said A1, A4, A3 where the list order said A1, A3, A4, and the printer ran the former. A printer that reports its slots itself, a P2S or an X1, keeps that report; a print started from the SD card or repeated from the printer's screen has no echo and keeps the estimate
...
Version 1.3.0-dev.16
   - Features:
      - A 3rd party spool can be assigned without asking, where there is nothing to ask (issue #47). "Assign 3rd party spools automatically" in the Tracking settings, off by default, links a spool without an RFID tag to the one Spoolman spool of the same material and colours that carries no tag and sits in no other slot. The slot then reads "auto-assigned", consumption is booked onto that spool, and the assignment is dropped like a manual one when a different filament shows up in the slot
         ...
         - The assignment picker preselects the same spool, so the usual case with the setting off is one click. The picker and the server decide with one function, public/match.js, so what the picker preselects is what the service would have assigned
         - The detail dialog says "assigned automatically" rather than "assigned by hand" for such a slot, and unassigning it and picking another makes it a manual one
      - A 3rd party spool is called by the preset chosen for its slot. A chipless spool showed "PLA" and the bare id, and the detail dialog called its id a "Tray profile", which it is not: a spool without a tag reports nothing of its own, only the filament preset somebody picked for the slot on the printer's screen or in the slicer. The slot now reads "Generic PLA preset", "SUNLU PETG preset" or "PLA · custom preset", and the dialog row is "Slot preset" with a line saying where it came from
         - Read off a P2S on 2026-09-06 by switching the preset of one chipless spool: a vendor added from Bambu Studio's filament library and a user's own preset both arrive as "P" and seven hex digits, the same value from the slicer and from the printer's screen once the printer is bound to the cloud, and it survives a restart. The name behind the hash is nowhere in what the printer reports, so it is shown as a custom preset with its hash. Choosing Bambu PLA Basic for the same spool made the printer report GFA00, the id of the real Bambu spools next to it, with the tag all zeros; the word "preset" stays in the name so that spool is not read as one
   ...
   - Fixes:
      ...
      - A print is matched to the right slot when a slot in the middle of an AMS is empty, on a printer that does not report where the print runs from (P1 and A1 series). Bambu Studio leaves an empty slot out of the filament list when it synchronises a project with the printer, so every filament after a gap moves up one position, while this service counted all four slots of every unit and looked one position too far to the right for each gap before it. Measured on a P2S emptied at A3 and B2, seven loaded slots and seven filaments, and confirmed on an X1E emptied at A3, whose own report named A4 for the third filament, exactly where the list order now puts it (issue #146)
         - The external spool holder also comes before an AMS HT in that list rather than after it, read off a P1S carrying both, although the printer numbers the holder 254 and an HT 128
...
Version 1.3.0-dev.14
   - Fixes:
      - A dual nozzle printer's second external spool holder is a slot of its own, "External-2". An H2C, H2D or X2D reports two holders, and both were labelled "External", so two loaded holders were two slots under one label, one assignment key and one Spoolman location. "External" keeps its meaning and feeds the first extruder, so nothing moves for a printer with one holder
         ...
         - A printer with one holder reports it under either number, 255 on a P2S and an X1C, 254 on an A1, a P1P and a P1S, so the label goes by how many holders are reported and not by the number: one holder is "External" whatever the printer calls it. Between two dev builds this was wrong for the 254 group and their holder would have come up as "External-2"
      - The README lists the A2L as untested. Its report in test/fixtures/reports carries an AMS as unit 16, outside every range the slot labels know, so all four slots come out as "Z" and cannot be assigned. Left as it is until a second report says what 16 is, rather than given a label that would have to migrate later
      - A P1 outside a print no longer shows "Stage 255". It reports 255 where an X1 and a P2S report -1, and both now read as no stage at all
   - Features:
      - A print error is named, not only numbered. The summary of a failed or stopped print said "Printer error 50348044"; it says "Printer error 50348044: The task was canceled." now. The number stays in front, because it is what a bug report and Bambu's own lookup go by
         - The sentences are Bambu Lab's own. The dev.12 entry said the catalogue was not published in a form this project could carry, and that was wrong: the lookup Bambu Studio and the Handy app use, e.bambulab.com, answers for every printer model by serial prefix. scripts/fetch-print-errors.js fetches it for the nine known models and writes src/data/print-errors.json, 964 codes, English, majority wording where models differ. The file ships in the image and nothing is fetched at runtime, so the summary names the error while the network is down too
         - A code the catalogue does not know stays the bare number, as before. The HMS notifications in print.hms are a separate catalogue three times the size and are not read by this service, so they are not carried
      - The diagnostics download asks which logs to include: the server log and each printer separately, all ticked to begin with. The configuration files are in every bundle, the choice is over the logs alone, and info.json says which were asked for, so an archive without a printer's log reads as a choice rather than as a printer that never logged. A raw MQTT trace runs at about 22 MB an hour per printer, and an installation with several printers usually has a question about one of them. A script gets the same choice through the scope query of /api/diagnostics/download
         - The log detail dialog of a printer, "Log" in the Printers card, carries an export of its own at the bottom: the printer log and the raw MQTT trace ticked separately, then the same archive with only those. The scope query names them as <serial>/log and <serial>/trace
      - Print stages 36 to 77 are named. They are the laser, cutter and camera calibrations of the H2 series, the hotend swap of the X2D, the chamber and heatbed steps and "Preparing AMS", following the codes ha-bambulab carries; a P2S never reports them, a printer that does says what it is doing rather than "Stage 63". They count as preparing, except the two that cool something down at the end of a job. Anything above 77 still falls back to its number
   - Maintenance:
...
Version 1.3.0-dev.13
   - Breaking:
      - "Debug logging" is a log level now, not a switch. The Logging card carries "Log detail...", which opens the level, the areas that write and the raw MQTT capture in one dialog
         ...
         - A stored DEBUG is migrated on the first start: true becomes the debug level and false becomes normal. Both are written, including the false, because a value in that file is one somebody saved and the file beats the environment variable. Dropping the false would have let a DEBUG=true in the container definition seed the level on the next start and turn debug logging back on against an explicit decision. The variable still seeds an installation that has never saved a level
   - Fixes:
      ...
      - A clean print no longer inherits the failure of the one before it. The summary of a FINISH said "Fail reason 50348044", the code of a print that had failed two days earlier
         - The printer does not clear its complaint when the state changes. Read out of the raw MQTT capture on a P2S: the report that first said RUNNING still carried the old fail_reason, and only the next report, five seconds later, had it back at 0. The error is collected across reports, on purpose, because at the end of a print the code arrives one report after the state; the same lag at the start meant the first report of a new print handed it the old one
         - The complaint a print starts with is now held back for as long as the printer keeps repeating it. The moment it reports anything else, including nothing, whatever comes after belongs to this print. An error that really happens again inside the print is still collected, once the old one has been cleared even briefly
      - Two print stages are named instead of shown as a number: 51 is the calibration lines and 54 is the heatbed coming up to temperature. Both are what a P2S announces on an ordinary plate, which is why they were the two anybody saw as "Stage 51" and "Stage 54". They count as preparing, so the badge is amber: the calibration lines go next to the plate, not on it, and a bed still heating has not started. Everything else nothing has named still falls back to its number
   - Features:
      - Every MQTT report a printer sends can be captured into a file of its own, "Capture raw MQTT messages" in the log detail dialog. It is what the printer really sent rather than what this service made of it, which is what a bug report about behaviour nobody can reproduce on demand needs
         ...
         - Written one line per message into logs/<serial>.mqtt.log, with its own size and history budget: at the log file's default of 1 MB a capture would evict the whole log within minutes. Measured on a P2S, idle and through a whole print, it runs at about 22 MB an hour either way, so the default of 50 MB with 2 kept files is roughly six hours of history and the dialog says what an hour costs
         - It is every message, not only the status reports this service reads. A P2S print produced seven kinds: push_status, gcode_line, project_file, xyz_ctrl, ledctrl, get_version and get_sn. Only the first is looked at anywhere in this service, so the other six had never been visible
         - The report is written as it arrived except for the printer's own line breaks and the indentation behind them, which is half of what a P2S sends: 17 KB a report became 8 KB. Only the run that follows a line break is folded, because a raw line break cannot appear inside a JSON string while two spaces inside a job name can
         - It stays on until it is switched off. Nothing turns it off by itself, because a fault that shows up twice a day is not caught by a capture that ended an hour ago
      ...
      - The print path says what it is doing, which it never did before: it had no debug output at all, only the handful of [Print] lines on the ordinary level. The FTPS fetch names the paths it looks for, which one answered and with how many bytes, and whether the archive carried the settings entry that holds a multi colour set. The booking names every state change, which of the two consumption sums ran and on what layer, whether the slots came from the printer or were estimated from the slicer's list order, which slots may carry a booking at all, and what the matcher answered per filament
...
Version 1.3.0-dev.12
   - Features:
      - A finished print leaves a summary behind. "consumption booked" is a button now, and it opens what the print actually did: the result, when it started and ended, how long it took, how many layers, and one row per filament with the slot it ran from, the grams and the spool they were booked onto
         ...
         - A filament that was not booked is named from the spool in its slot, but only where the printer itself named that slot in print.mapping, so the sliced file and the printer agree on it. Where the slot is only the slicer's list order, which is the estimate the booking refuses to act on without confirming, the row falls back to what the sliced file knew, a material and a hex code. Its spool column stays empty either way: that column says what carried the booking, and this spool carried none
         - An error the printer names is carried into the summary, including the code a print stopped by hand reports. It does not arrive with the report that ends the job: measured on a P2S, the state went to FAILED first, the code came one report later and was back at 0 the report after that, so it is collected across reports and added to the summary when it turns up. The number is shown as the printer gave it, because Bambu's error catalogue is not published in a form this project can carry
      ...
      - The card says more about a print while it is running: when it started, how long it has been going and when it is expected to end, next to the layer progress that was already there
         - The expected end comes from the remaining time the printer reports and is revised as it goes. The start time is this service's own measurement, because the printer reports none: a service restarted mid print leaves it out rather than inventing one
         - How long it has been running counts up in the same clock as the countdown next to the booking label. The start and the expected end follow the same rule as the summary: the times alone while the whole job falls on today, the full date once it runs over midnight
         - The time left is stated at the precision the printer actually has. It reports whole minutes and revises them as it goes, so it reads "~ 3 min" or "~ 6 Days 4 hours 30 min" rather than a clock claiming a second hand the number does not have
         - A paused print shows no expected end at all, only what it still needs once it is resumed. Adding the remaining work to the clock named a moment that moved further away for as long as the pause lasted, so it was wrong every time anybody read it
         - The stage badge stays empty while the printer is simply printing. Measured on a P2S: it reports stage 0 for the whole body of a print, so naming it put "RUNNING" and "Printing" side by side for almost the entire job, and "PAUSE" next to "Printing" during a pause
         - A badge next to the state names what the printer is busy with when it is not laying down filament: heating the bed, homing, levelling, calibrating, loading. It is amber while the printer is getting ready, so a job that has started but is not printing yet says so instead of looking stalled. PREPARE gets the same amber badge
         - The stage names are the ones the community has settled on, not a published table, so a number nobody has a name for is shown as "Stage 54" rather than guessed at. A P2S announces stages 51 and 54 for an ordinary plate
      - The card returns to idle on its own once a print has been over for a while. The state badge, the job name, the booking label and the "Needed" and "After print" columns all go back to what they show when nothing is printing
         ...
         - Next to the booking label is a Clear button carrying the remaining time in brackets, "Clear (09:24)", which clears the result now instead of waiting the countdown out. Tabular figures keep the digits from jittering as it counts. Every open dashboard follows at once rather than waiting out its update interval. It is a button like the ones in the tables, because it changes the dashboard, unlike the booking label beside it, which only opens a description of what already happened

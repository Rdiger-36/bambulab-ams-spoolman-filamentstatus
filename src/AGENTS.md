...
| `printerrors.js` | Bambu Lab's error catalogue as `data/print-errors.json`, read once at import, and `describePrintError()`, the number a printer reports turned into the catalogue's sentence. The file is regenerated by `scripts/fetch-print-errors.js` from Bambu's own lookup endpoint and shipped, never fetched at runtime. |
...
`config.js` when it should be seedable, and document it in the settings card
table in `docs/settings.md`. The settings page picks it up on its own; `group`
decides which card it lands in, and the card has to be listed in `GROUPS` in
...
optional; the shapes that have been observed are in `test/ams.env.test.js`.
